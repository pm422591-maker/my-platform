...
if (result.success) {
    ...
    // Обучение и тест — ТОЛЬКО для новых аккаунтов.
    // Сигнал дублируем: sessionStorage + URL (?welcome=1) на случай блокировки storage.
    if (result.is_new) {
...
if (formLogin) {
    ...
    body: JSON.stringify({ email, password })
            ...
            if (result.success) {
                ...
                // Вход существующего юзера НЕ запускает обучение.
