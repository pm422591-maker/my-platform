<?php
// upload_background.php — ЗАХИЩЕНА ВЕРСІЯ (за зразком upload_avatar.php)
error_reporting(0);
...
// ── Цей скрипт приймає тільки задній фон
$fileKey   = 'background';
...
try {
    ...
    $stmt = $pdo->prepare("UPDATE users SET `background_url` = ? WHERE id = ?");
    $stmt->execute([$dbUrl, $userId]);

    echo json_encode(['success' => true, 'url' => $dbUrl]);

} catch (Exception $e) {
    error_log('Upload background DB error: ' . $e->getMessage());
